...
class StaffCalendarWidget extends CalMeWidget
{
        ...
        return $schema->schema([
            ...
            Select::make('company_id')
                ...
                ->afterStateUpdated(function (Set $set, ?string $state): void {
                    $set('staff_ids', Staff::where('company_id', $state ?? null)->where('is_active', true)->pluck('id')->toArray());
                }),

            CheckboxList::make('staff_ids')
                ->label('Staff')
                ->default(fn (Get $get) => Staff::where('company_id', $get('company_id'))->where('is_active', true)->pluck('id')->toArray())
                ->options(fn (Get $get) => Staff::where('company_id', $get('company_id'))->where('is_active', true)->get()->mapWithKeys(fn (Staff $s) => [$s->id => $s->full_name]))
                ->bulkToggleable()
                ->live(),
        ]);
    }
    ...
    protected function getResources(): array
    {
        return Staff::query()
            ...
            ->whereIn('id', $this->filters['staff_ids'] ?? [])
            ->where('is_active', true)
            ->get()
            ->mapWithKeys(fn (Staff $staff) => [$staff->id => mb_strtoupper(mb_substr($staff->first_name, 0, 1)).'. '.$staff->last_name])
            ->toArray();
    }

    /**
     * @return array<int, array{avatar_url: string|null, color: string|null, initials: string}>
     */
    #[Computed]
    public function getResourceMeta(): array
    {
        return Staff::query()
            ...
            ->mapWithKeys(fn (Staff $staff) => [
                $staff->id => [
                    'avatar_url' => $this->resolveAvatarUrl($staff->avatar_url),
                    'color' => $staff->color,
                    'initials' => mb_strtoupper(mb_substr($staff->first_name, 0, 1).mb_substr($staff->last_name, 0, 1)),
                ],
            ])
            ->toArray();
    }
                    ...
                    Fieldset::make('Booking Details')
                        ->schema([
                            ...
                            Select::make('staff_id')
                                ->label('Staff')
                                ->options(Staff::where('company_id', $companyId)->where('is_active', true)->get()->mapWithKeys(fn (Staff $s) => [$s->id => $s->full_name]))
                                ...
                                ->rules([
                                    ...
                                    function (Get $get, ?Model $record) {
                                        return function (string $attribute, $value, Closure $fail) use ($get, $record): void {
                                            ...
                                            $query = Reservation::where('staff_id', $staffId)
    ...
    protected function getEvents(): Collection
    {
        ...
        return Reservation::query()
            ...
            ->when(
                $this->filters['staff_ids'] ?? null,
                fn ($query, $staffIds) => $query->whereIn('staff_id', $staffIds)
            )
            ->get();
    }
